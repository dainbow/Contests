...
int main() {
    ...
    scanf("%u %u", &nAmount, &kAmount);

    createHeap(heapsArr, 0, kAmount + 1, 0);
    ...
    for (uint32_t curNumber = 0; curNumber < nAmount; curNumber++) {
        ...
        if (curNumber >= kAmount)
            removeTop(heapsArr, 0);
    }

    int64_t* answArr = calloc(kAmount, sizeof(int64_t));
    int32_t curElem = 0;

    while (heapsArr[0].heapLast) {
        answArr[curElem++] = heapsArr[0].heapArr[0];

        removeTop(heapsArr, 0);
    }

    curElem--;
    for (;curElem >= 0; curElem--) {
        printf("%lu\n", answArr[curElem]);
    }
